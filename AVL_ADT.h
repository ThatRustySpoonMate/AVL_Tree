...
class AvlTree
	{
	 protected:
		...
		void  _search(string TargetText,
			NODE<TYPE>* root);
...
//  	The following function is used for debugging.
	 ...
	 public:
		...
		void AVL_Search(string TargetText);
...
template <class TYPE, class KTYPE>
void  AvlTree<TYPE, KTYPE>
::AVL_Search(string TargetText)
{
	_search(TargetText, tree);
	return;
}

template <class TYPE, class KTYPE>
void  AvlTree<TYPE, KTYPE>
::_search(string TargetText,
	NODE<TYPE>* root)
{
	if (root)
	{
		if (root->data.getTitle().find(TargetText) != string::npos) {
			root->data.printAItem(3);
		}
		_search(TargetText, root->left);
		_search(TargetText, root->right);
	}
	return;
}
